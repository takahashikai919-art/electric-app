...
export default function Home() {
  ...
  const companies:any = {
    ...
    "トドック電力":[
      {name:"ベーシック",type:"tier",t1:34,t2:40,t3:44, note:"363円(税込)につき1pt / トドック手数料無料"},
      {name:"再エネ",type:"tier",t1:36,t2:42,t3:46, note:"環境配慮型 / ポイント付与あり"},
      {name:"スタンダード",type:"tier",t1:34,t2:40,t3:44, note:"標準プラン"},
      {name:"灯油セット",type:"tier",t1:32,t2:38,t3:42, note:"灯油とセットで割引"},
      {name:"トリプルセット",type:"tier",t1:31,t2:37,t3:41, note:"電気＋灯油＋他サービス"}
    ],
  ...
  if(card==="platinum"){
    if(platinumUse==="low") greenRate+=0.02;
    if(platinumUse==="mid") greenRate+=0.04;
    if(platinumUse==="high") greenRate+=0.06;
  }
  ...
  return (
    <div className="p-4 max-w-md mx-auto bg-white rounded shadow">
      ...
      <div className="flex gap-2 mb-2">
        {["night","normal","day"].map(m=>(
          <button key={m}
            onClick={()=>setMode(m)}
            className={`px-2 border ${mode===m?"bg-blue-500 text-white":""}`}>
            {m==="night"?"夜多め":m==="day"?"昼多め":"標準"}
          </button>
        ))}
      ...
      {/* 季節 */}
      <div className="flex gap-2 mb-2">
        {["winter","normal","summer"].map(s=>(
          <button key={s}
            onClick={()=>setSeason(s)}
            className={`px-2 border ${season===s?"bg-red-500 text-white":""}`}>
            {s==="winter"?"冬":s==="summer"?"夏":"通常"}
          </button>
        ))}
      ...
      <div className="bg-yellow-100 p-3 text-center mb-3">
        ...
        <p>Basic：{docomoBasic.toFixed(0)}円（月{diffBasic.toFixed(0)}円 / 年{(diffBasic*12).toFixed(0)}円）</p>
        <p>Green：{docomoGreen.toFixed(0)}円（月{diffGreen.toFixed(0)}円 / 年{(diffGreen*12).toFixed(0)}円）</p>
      </div>
