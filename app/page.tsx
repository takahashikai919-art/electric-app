...
export default function Home() {
  ...
  const calcBaseHokuden = () => {
    const base =
      { 20: 759, 30: 1138, 40: 1518, 50: 1897, 60: 2277 }[amp] || 1138;
  ...
  const companies: any = {
    ...
    ENEOSでんき: [
      { name: "標準", calc: () => baseTable[amp] + calcTier(kwh, 34, 40, 44) },
    ],
  ...
  const currentCost = selectedPlan?.calc ? selectedPlan.calc() : 0;
  ...
  ranking.push({
    name: "ドコモBasic（ポイント充当後）",
    cost: docomoBasic,
  });
  ranking.push({
    name: "ドコモGreen（ポイント充当後）",
    cost: docomoGreen,
  });
    ...
    <div className="p-4 max-w-md mx-auto bg-white rounded shadow">
      ...
      <div className="bg-yellow-100 p-3 text-center mb-3">
        ...
        <p className="mt-2">
          ...
          月{(currentCost - docomoBasic).toFixed(0)}円 / 年
          {((currentCost - docomoBasic) * 12).toFixed(0)}円
        </p>

        <p className="mt-2">
          Green → {docomoGreen.toFixed(0)}円（dポイント充当後）<br />
          月{(currentCost - docomoGreen).toFixed(0)}円 / 年
          {((currentCost - docomoGreen) * 12).toFixed(0)}円
        </p>
      </div>
